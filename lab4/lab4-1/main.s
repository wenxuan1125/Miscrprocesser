...
.text
	...
	.equ INTENSITY, 0x0A
	.equ SHUTDOWN, 0x0C
	.equ DIN, 0x10	//PB4 0b10000
	.equ CS, 0x20	//PB5 0b100000
	.equ CLK, 0x8 //PB3 0b1000
...
max7219send:
	...
	ldr r1, =#CS	// information in cs, din and clk are about their port number
	ldr r2, =#DIN
	ldr r3, =#CLK
	ldr r4, =#GPIOB_BSRR
	ldr r5, =#GPIOB_BRR
...
max7219send_loop:
	...
	str r3, [r5]	// use brr to set the clk ( port output) = 0

	// test whether r0 AND¡@r7 ( the value in bit i in r0) is 0,
	// if it is 0, use brr to set din = 0 ( branch to bit_not_set)
	// if it is 1, use bsrr to set din = 1
	// after setting value, check whether all 16 bits have sent ( if_done)
	tst r0,  r7
	beq bit_not_set
...
if_done:
	str r3, [r4]	// use bsrr to set the clk ( port output) = 1, making it rising edge
	subs r6, r6, #1
	bgt max7219send_loop

	// use brr to set the cs ( port output) = 0
	// then use bsrr to set the cs ( port output) = 1, making it rising edge
	str r1, [r5]
	str r1, [r4]
...
max7219_init:
	...
	ldr r0, =#SCAN_LIMIT
	ldr r1, =#0x0	// display digit 0 only
	bl max7219send
